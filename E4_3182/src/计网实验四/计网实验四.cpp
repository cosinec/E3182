...
int main()
{
	...
	*/
	...
	/* 获得设备列表 */
	char* p = new char[strlen(PCAP_SRC_IF_STRING) + 1];
	p = strcpy(p, PCAP_SRC_IF_STRING);
...
void output(ip_header * ih, mac_header* mh, const struct pcap_pkthdr *header, char user[], char pass[], bool isSucceed)
{
	...
	*/
	...
	//其他格式
	FILE* fp1 = fopen("out1.csv", "a+");
	fprintf(fp1, "%s,", timestr);//时间
	...
	fprintf(fp1, "%s,%s,", user, pass);//账号密码
